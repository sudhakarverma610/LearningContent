/* Create DIMS database.                                                                     */

use master;

go

IF EXISTS(select * from sys.databases where name='DIMS')
drop database "DIMS"
...
alter table "ordermaster"
	add constraint "employee_ordermaster_FK1" foreign key (
		"emp_id")
	 references "employee" (
		"emp_id") on update no action on delete no action  

go

alter table "ordermaster"
	add constraint "customer_ordermaster_FK1" foreign key (
		"customer_id")
	 references "customer" (
		"customer_id") on update no action on delete no action  

go

/* Add foreign key constraints to table "orderdetail".                                        */
alter table "orderdetail"
	add constraint "ordermaster_orderdetail_FK1" foreign key (
		"order_id")
	 references "ordermaster" (
		"order_id") on update no action on delete no action  

go

alter table "orderdetail"
	add constraint "product_orderdetail_FK1" foreign key (
		"product_id")
	 references "product" (
...
alter table "employee"
	add constraint "department_employee_FK1" foreign key (
		"dept_id")
	 references "department" (
		"dept_id") on update no action on delete no action  
