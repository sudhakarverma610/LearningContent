...
export class AddAddressComponent implements OnDestroy, OnInit {
  ...
  constructor(
    ...
    public data: { address: AddressesItem; type: number }
  ) {
  ...
  ngOnInit() {
    this.service.getStates().subscribe(value => {
      if (value) {
        this.states = value;
      }
    });
  ...
  saveAddress() {
    if (this.isValid) {
      this.service.addAddress(this.form.value).subscribe(res => {
        if (res) {
